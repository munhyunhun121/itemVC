...
class mapVCViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            if tableView == self.tableView {
                ...
                customers2.remove(at: indexPath.row)
                saveCustomers2ToUserDefaults()
            }
    ...
    func showRegisterAlert2() {
        ...
        let saveAction = UIAlertAction(title: "저장", style: .default) { [weak self] _ in
            ...
            //      각각의 textFields에 저장된 값들을 대입해주고 뉴커스터머라는 상수에 넣어준다.
            let newCustomer1 = Customer(name: name, phoneNumber: phoneNumber, birthdate: birthdate, checkMonth: checkMonth)
            
            // customers 변수 배열에 넣어준다.
            self.customers2.append(newCustomer1)
